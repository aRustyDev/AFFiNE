...
> Tasks 0–5 done and verified (`PLAN.md` carries the as-built notes). 39 tests pass;
...
| Change absent from any upstream-directed branch — asserted by `woven-manifest-guard.sh --outbound --head HEAD` **naming `core/quota/state.ts`** in its LEAKED list. Not by its exit code: `oidc.ts` is a pre-existing core patch, so the guard exits 1 on any branch of this fork regardless of whether our row is right | 5   |
