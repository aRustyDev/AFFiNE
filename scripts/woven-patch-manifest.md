...
| `packages/backend/server/package.json`                                     | **ADDITIVE**              | The `predeploy` npm script is a SEPARATE code path from `self-host-predeploy.js` above: `.github/helm/affine/charts/graphql/templates/migration.yaml` (the in-repo Helm chart) runs `yarn predeploy` directly, and so would upstream's own chart or `render.yaml` or a bare `yarn predeploy`. Upstream's definition was `yarn prisma migrate deploy && yarn cli run` — no compatibility check, no stamp, so this path migrated and ran ungated. Changed to `yarn cli db check && yarn prisma migrate deploy && yarn cli run && yarn cli db stamp` to close that hole, fixing the script itself rather than repointing it at `self-host-predeploy.js` (which also generates a private key the cloud chart should not get).                                                                                                                                                                                                                                                                                                                                                                 | upstream grows its own boot-time schema-compatibility check                                                                                                                                                                                                                                                                                 |       |
...
| `packages/backend/server/src/plugins/indexer/service.ts`                   | **ADDITIVE**              | Defensive-only bugfix in `searchDocsByKeyword`, bead `affine-adi`: every aggregate-hit field read is optional-chained before its `[0]` subscript. Blocks indexed before upstream `ee899a267b` (#15448) carry none of that commit's fields (`unitId`, `projectionVersion`, `sourceHash`, `visibility`); the provider omits those keys entirely, so a bare subscript threw `Cannot read properties of undefined (reading '0')` and failed the WHOLE query on one stale row. Measured on the woven deployment 2026-09-03: zero-hit searches returned `[]`, any search WITH a hit returned INTERNAL_SERVER_ERROR, and a raw `search` asking for those seven fields came back with only `docId`/`blockId`/`flavour` present. `elementId`/`frameId`/`sourceBlockId`/`highlights` were ALREADY guarded here — the inconsistency was the bug. A healthy index behaves identically, so no intended upstream behavior changes; ADDITIVE for the same reason the `build-test.yml` row is, and a plausible upstream contribution, since upstream carries the same latent bug on any pre-#15448 index. | upstream guards these reads itself, or the fix is accepted upstream. NOT when the index is reindexed — a reindex clears today's stale rows, but the next schema-adding commit reintroduces the crash. The companion drift detection below repairs the DATA; these guards keep a single stale row from failing the whole query while it does |       |
| `packages/backend/server/src/plugins/indexer/providers/manticoresearch.ts` | **ADDITIVE**              | Adds one public method, `listTableColumns(table)`, reading `DESCRIBE <table>` so `IndexerService.repairManticoreSchemaDrift` can compare the live schema against the Zod table schemas (bead `affine-adi`). Uses `/sql?mode=raw`, which returns JSON, rather than the `/cli` endpoint the existing private `#executeSQL` uses, which returns a pipe-delimited text table that would have to be parsed positionally; response shape measured against Manticore 10.1.0 on the deployment 2026-09-08. Purely additive — no existing method is touched and no upstream behavior changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | upstream detects its own Manticore schema drift, or exposes a column list of its own                                                                                                                                                                                                                                                        |       |
| `packages/backend/server/src/plugins/indexer/job.ts`                       | **ADDITIVE**              | Calls `repairManticoreSchemaDrift()` at the top of the existing `indexer.autoIndexWorkspaces` handler, inside a try/catch so a failed check cannot stop indexing (bead `affine-adi`). Hooked into a queued job rather than application bootstrap on purpose: a job runs once per tick on one consumer, whereas a bootstrap hook would have every replica racing to drop and rebuild the same tables on restart. A healthy schema costs two `DESCRIBE` calls per tick and nothing else. Adds a call, changes no existing logic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | drift detection moves upstream, or upstream gains a boot-time schema reconciliation                                                                                                                                                                                                                                                         |       |
