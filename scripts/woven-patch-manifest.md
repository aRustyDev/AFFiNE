...
| `.husky/pre-commit`                                           | **ADDITIVE**              | Adds `#!/usr/bin/env sh`, nothing else. Husky 9 hooks are shebang-less by design: its installer generates `.husky/_/` wrappers that supply the shell. That installer runs from the root `postinstall`, which `.yarnrc.yml` disables via `enableScripts: false`, so `core.hooksPath` resolves to raw hook files instead. An explicit shebang makes the hook runnable where git does not fall back to `sh`. No behavior change, low-conflict on future merges.                                                                                                                           | husky's installer runs (`.husky/_/` exists and `core.hooksPath` points at it), or upstream adds a shebang |
