...
| `.husky/pre-commit`                                           | **ADDITIVE**              | Adds `#!/usr/bin/env sh`, nothing else. Husky 9 ships its hooks shebang-less on purpose: its installer generates `.husky/_/` wrappers that supply the shell. That installer never runs here — it is in the root `postinstall` (`yarn affine init && yarn husky`), and `.yarnrc.yml` sets `enableScripts: false` — so `.husky/_` does not exist and `core.hooksPath` points straight at `.husky`. Git then execs a file with no shebang and dies with "Exec format error", which is why the hook was silently doing nothing and a formatting break reached CI. One line, no behavior change, low-conflict on future merges. | husky's installer actually runs (`.husky/_/` exists and `core.hooksPath` points at it), or upstream adds a shebang |
